...
const std::string switching_protocol = "HTTP/1.1 101 Switching Protocols\r\n";
...
asio::const_buffer toBuffer(Reply::status_type status) {
    switch (status) {
        case Reply::switching_protocol:
            return asio::buffer(switching_protocol);
        case Reply::ok:
...
void Reply::send(status_type status) {
    status_ = status;
    bool omittContentLength = status < 200 || status == 304 || status == 204;
    if (!omittContentLength) {
