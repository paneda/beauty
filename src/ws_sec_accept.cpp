...
// Minimal footprint sha1 taken from https://github.com/CTrabant/teeny-sha1
int sha1digest(uint8_t *digest, const char *data, size_t databytes) {
...
std::string WsSecAccept::compute(const char *key) {
    ...
    std::string totKey = std::string(key, keySize) + magicGUID;
    uint8_t digest[20];
