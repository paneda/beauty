...
// Callback function for streaming data - returns number of bytes written to buffer, 0 or negative for end of stream
typedef std::function<int(char* buf, size_t maxSize)> StreamCallback;
...
class Reply {
   ...
   public:
    ...
    void sendBig(status_type status, const std::string& contentType, size_t totalSize, StreamCallback callback);
